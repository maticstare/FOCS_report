...
Ker je pričakovana moč množice $|R_1| = O(\frac{m}{k})$ algoritem terja časovno zahtevnost $O(mk \log k)$. %Fix
...
\begin{enumerate}
  \item
  Ko iz kopice izločimo vozlišče $u$, posodobimo tabelo tako, da za vsako vozlišče $v$, ki je zapakirano vozlišču $u$ najdemo točno razdaljo. To storimo tako, da %izberemo minimum izrazov $d(v)$, $d(u) + dist(u,v)$, $d(y) + dist(y,v)$ in $d(z_1) + w_{z1z2} + dist(z_2,v)$
